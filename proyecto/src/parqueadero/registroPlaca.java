 ...
 * Posicion 4= segun tipo de vehiculo(tipo 1 hora de salida, tipo 2 suma de tiempo al total acumulado, tipo 3 importe a pagar) 
...
public class registroPlaca extends vehiculo{
    ...
    public void imprimir() {
    	for (int a=0; a<datosvehiculo.size();a++) {
    		System.out.println("Placa: "+datosvehiculo.get(a).getNroPlaca()+" hora entrada: "+datosvehiculo.get(a).getHoraEntrada()
    				+" tipo veh: "+datosvehiculo.get(a).getTipoVeh()+" hora salida: "+datosvehiculo.get(a).getHoraSalida()
    				+" Estado: "+datosvehiculo.get(a).getEstado());
    		}
    }
    ...
    public void SalidaVeh() {
    	...
    	if(busquedaPlaca()) {
			...
			case 1:	
				...
				break;
			case 2:
				dato=String.valueOf(diferenciaHoras(formateo(datosvehiculo.get(posicion).getHoraEntrada()), formateo(formatDate())));
				datosvehiculo.get(posicion).setHoraEntrada("0");
				datosvehiculo.get(posicion).setHoraSalida(String.valueOf(Integer.valueOf(datosvehiculo.get(posicion).getHoraSalida())+(Integer.valueOf(dato))));
				mensaje();
				break;
			case 3:	
				dato=String.valueOf(preciomin*diferenciaHoras(formateo(datosvehiculo.get(posicion).getHoraEntrada()), formateo(formatDate())));
				System.out.println("/**************************************/");
    ...
    public void escrituraArchivo() { 
    	borrar();
    	for(int a=0; a<datosvehiculo.size();a++) {
    		registro=datosvehiculo.get(a).getNroPlaca()+'@'+datosvehiculo.get(a).getHoraEntrada()+'@'+datosvehiculo.get(a).getTipoVeh()+'@'+datosvehiculo.get(a).getHoraSalida()
